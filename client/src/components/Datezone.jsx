...
const Datezone = (props) => {
    const { interested, interests, onDump } = props;
    console.log('datezone props', props);
    return (
        <div>
            <h2>You are in the Datezone!</h2>
            <div className="friend-div col-12 row">
                {interested.length > 0 ? interested.map((match, i) => <MatchOption match={match} key={i} interests={interests} onDump={onDump} />) : 'No Matches Found'}
            </div>
        </div>
    )
}
